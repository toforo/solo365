...
<#macro header type>
    ...
    <header id="headerBg" class="header header--${type}">
...
<script>
    ...
    function showBg() {
        ...
        let num = Math.floor(sec / step) % count;
        $("#headerBg").css("background", "url('" + srcs[num] + "') no-repeat center").css("background-size", "100% 100%");
    }
    function showBg2() {
        let count = 16; // 背景图数量
        let step = 60 * 5;   // 背景图切换频率(秒)
        ...
        let sec = Math.floor(new Date().getTime() / 1000);
        let num = Math.floor(sec / step) % count;
        $("#headerBg").css("background", "url('/skins/Bubble365/images/bg/bg-" + num + ".jpg') no-repeat center").css("background-size", "100% 100%");
    }
